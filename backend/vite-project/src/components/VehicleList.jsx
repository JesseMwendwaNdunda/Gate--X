...
function VehicleList() {
  ...
  const fetchEntries = async () => {
    try {
      const res = await fetch("/api/vehicle_entries", { headers: { "Content-Type": "application/json", Authorization: `Bearer ${token}` } });
  ...
  const deleteEntry = async (id) => { if (role !== "admin") return alert("Only admin can delete!"); try { const res = await fetch(`api/vehicle_entries/${id}`, { method: "DELETE", headers: { Authorization: `Bearer ${token}` } }); if (!res.ok) throw new Error("Failed to delete entry"); setEntries(prev => prev.filter(e => e.id !== id)); } catch (err) { console.error(err); alert("Error deleting entry"); } };
